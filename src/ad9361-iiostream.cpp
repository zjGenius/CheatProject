...
int sendIQData(const char *udp_send_ip, sockaddr_in &udp_Addr)
{
	...
	// }
	IIO_ENSURE((ctx = iio_create_context_from_uri("ip:192.168.10.226")) && "No context");
	...
	printf("%s,%d\n", __FILE__, __LINE__);
	int16_t *rec_data = (int16_t *)malloc(sizeof(int16_t) * 1024 * 1024 * 2);
	FILE *fid = NULL;
	FILE *fd = NULL;
	char dataname[8] = "iqdata";
	char filename[20] = {0};
	char filename_txt[20] = {0};
	...
	while (!stop)
	{
		...
		int data_num = 0;
		memset(rec_data, 0, 512 * 1024);

		printf("%s,%d\n", __FILE__, __LINE__);
		// sprintf(filename_txt, "iq%d.txt", looptime);
		// fd = fopen(filename_txt, "wb");
		...
		for (p_dat = (char *)iio_buffer_first(rxbuf, rx0_i); p_dat < p_end; p_dat += p_inc)
		{
			...
			((int16_t *)p_dat)[0] = q;
			((int16_t *)p_dat)[1] = i;
			rec_data[data_num] = p_dat[0];
			rec_data[data_num + 1] = p_dat[1];
			data_num++;
			// fprintf(fd, "real: %d , imag: %d\n", p_dat[0], p_dat[1]);
		}
		// 写文件
		printf("%s,%d\n", __FILE__, __LINE__);
		sprintf(filename, "iq%d.dat", looptime++);
		printf("%s,%d\n", __FILE__, __LINE__);
		fid = fopen(filename, "wb");
		fwrite(rec_data, sizeof(int16_t), data_num, fid);
		fflush(fid);
		fclose(fid);
		...
		pack_num = nbytes_rx / BUFF_SIZE;
		for (int n = 0; n < pack_num; n++)
		{
			memset(&IQ_data, 0, sizeof(IQ_PACK));
			IQ_data.head = PACK_HEAD;
			IQ_data.pack_num = pack_num;
			IQ_data.num = n + 1;
			memcpy(IQ_data.data, rec_data + n, BUFF_SIZE);
